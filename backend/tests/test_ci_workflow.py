...
class CIWorkflowTest(unittest.TestCase):
    def test_ci_workflow_runs_backend_and_frontend_checks(self) -> None:
        ...
        self.assertIn("backend/common/export_logic.py", text)
        self.assertIn("backend/flux_service/runtime.py", text)
        self.assertIn("backend/flux_service/main.py", text)
        self.assertIn("backend/gateway/init_provider.py", text)
        self.assertIn("backend/gateway/projects.py", text)
        self.assertIn("backend/gateway/benchmarks.py", text)
        self.assertIn("backend/gateway/security.py", text)
        self.assertIn("backend/gateway/deployment.py", text)
        self.assertIn("JOBS_DIR", text)
        self.assertIn("BENCHMARKS_DIR", text)
        self.assertIn("GATEWAY_API_TOKEN", text)
        self.assertIn("Import backend modules", text)
        self.assertIn("import common.export_logic", text)
        self.assertIn("import flux_service.runtime", text)
        self.assertIn("import flux_service.main", text)
        self.assertIn("import gateway.init_provider", text)
        self.assertIn("import gateway.projects", text)
        self.assertIn("import gateway.benchmarks", text)
        self.assertIn("import gateway.security", text)
        self.assertIn("import gateway.deployment", text)
        self.assertIn("import gateway.main", text)
        self.assertIn("npm install", text)
        self.assertIn("node tests/apiClient.test.mjs", text)
        self.assertIn("node tests/benchmarkState.test.mjs", text)
        self.assertIn("node tests/initCandidates.test.mjs", text)
        self.assertIn("node tests/exportState.test.mjs", text)
