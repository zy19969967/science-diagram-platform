...
import { Activity, BarChart3, Download, FolderOpen, History, Image, Save, Sparkles } from "lucide-react";
...
import { INSPECTOR_TABS, WORKSPACE_COPY } from "../uiPresentation.js";
...
function ResultPanel({
  ...
  const [activeInspectorTab, setActiveInspectorTab] = useState("result");
  ...
  const inspectorTabs = INSPECTOR_TABS.map((tab) => ({ ...tab, icon: inspectorIcons[tab.value] }));

  return (
    <aside className="workbench-panel result-panel whiteboard-inspector">
      <div className="panel-heading result-heading inspector-heading">
        <div>
          <h2>{WORKSPACE_COPY.inspectorTitle}</h2>
        </div>
        <span className={latestResult ? "status-pill compact success-state" : "status-pill compact"}>{latestResult ? "有结果" : "待生成"}</span>
      </div>

      <div className="inspector-tabs segmented-grid" role="tablist" aria-label="结果面板">
        {inspectorTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={activeInspectorTab === tab.value ? "segment active" : "segment"}
      ...
      {activeInspectorTab === "result" && (
        <div className="inspector-tab-panel result-tab-panel">
          <section className="surface-block emphasis-block preview-block inspector-section">
            ...
            <div className="result-preview">
              {latestResult ? <img src={latestResult.result_image} alt="生成结果" /> : <div className="placeholder-card">结果会显示在这里</div>}
            </div>
            {latestResult && (
              <button type="button" className="secondary-button full-width icon-button-label" onClick={() => continueFromHistory(latestResult)}>
          ...
          {initCandidates.length > 0 && (
            <section className="surface-block rail-block candidate-block inspector-section">
              <div className="section-header compact-header">
                <div>
                  <span className="section-label">初图</span>
                  <strong>候选图</strong>
                </div>
                {initPlan && <span className="section-meta">{initSummary.fallbackUsed ? "回退候选" : "候选就绪"}</span>}
              </div>
          ...
          {activeJobSnapshot && (
            <section className="surface-block rail-block job-block inspector-section">
              <div className="section-header compact-header">
                <div>
              ...
              <div className="job-progress">
                ...
                {(smartJobSnapshot?.metadata?.fallback_used || smartJobSnapshot?.metadata?.is_diagnostic_result) && (
                  <p className="warning-text">当前结果是诊断占位，不代表正式模型效果。</p>
                )}
                {canCancelJob && (
          ...
          {latestResult?.evaluation && (
            <details className="surface-block rail-block result-details inspector-section" open>
          ...
          <details className="surface-block rail-block result-details inspector-section">
            <summary className="details-summary">
            ...
            <div className="history-list">
              {history.length === 0 && <div className="placeholder-card">还没有历史结果。</div>}
              {history.map((item) => (
      ...
      {activeInspectorTab === "project" && (
        <div className="inspector-tab-panel project-tab-panel">
          <details className="surface-block rail-block project-block inspector-section" open>
            <summary className="details-summary">
            ...
            <div className="action-row split-actions">
              <button
                type="button"
                className="secondary-button full-width icon-button-label"
      ...
      {activeInspectorTab === "export" && (
        <div className="inspector-tab-panel export-tab-panel">
          {(textValidationReport || svgExport) ? (
            <section className="surface-block rail-block export-block inspector-section">
              <div className="section-header compact-header">
                <div>
              ...
              {svgExport && (
                <button type="button" className="secondary-button full-width icon-button-label" onClick={() => downloadSvgExport(svgExport)}>
            ...
            <div className="placeholder-card inspector-empty">校验文本或导出 SVG 后，这里会显示导出状态。</div>
          )}
        </div>
      )}

      {activeInspectorTab === "benchmark" && (
        <div className="inspector-tab-panel benchmark-tab-panel">
          <details className="surface-block rail-block result-details inspector-section" open>
            <summary className="details-summary">
