...
当前已经补入无图文本入口的 API 合同和确定性 fallback 初图候选，便于前端先进入“文本初图 -> 后续局部编辑”的闭环。但这还不是报告中的 FLUX.2 [klein] 4B 初图服务，也没有真实多候选重排、低清预览/高清异步二阶段生成和矢量文字层同步；fallback 画面中的文字是位图提示，不是可编辑文字层。
...
仓库当前只有少量后端单元测试，还没有系统化后端测试、接口集成测试、前端端到端测试，也没有 CI 流水线来自动验证构建和部署。
