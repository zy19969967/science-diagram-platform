...
**Goal:** Bring the project closer to the technical report by implementing the first shippable dual-entry milestone and documenting the remaining phases.

**Architecture:** Phase 1 adds pure-text initial canvas contracts and a deterministic fallback renderer behind gateway endpoints. The generated initial canvas becomes the existing `sourceImage`, so all later local editing continues through the current Qwen/SAM2/PowerPaint loop.
