...
- Phase 11 completed in the current branch after Phase 10: configurable FLUX-compatible initial-canvas provider via `FLUX_INIT_URL`, provider selection in `InitGenerateRequest`, candidate scoring/reranking, explicit fallback policy, and frontend provider/score display. The original Phase 11 slice only shipped remote compatibility; the later local FLUX correction adds the in-repo service and local deployment wiring. This still does not claim bundled FLUX weights, high-resolution async init regeneration, or durable initial-candidate artifact storage yet.
...
- Phase 13 completed in the current branch after Phase 12: optional single-token gateway protection via `GATEWAY_API_TOKEN`, frontend `VITE_API_TOKEN` API header support, `/api/deployment/readiness` local deployment checks, CI coverage for the new hardening modules, and `docs/report-traceability.md` mapping Phase 1-13 report claims to code paths, tests, and limitations. This intentionally does not claim multi-user login, RBAC, token rotation, external service uptime probes, Docker/GPU smoke tests, or production observability.
- Post-alignment documentation refresh completed after Phase 13: `README.md`, `docs/architecture.md`, Docker/Conda deployment docs, and `docs/known-issues.md` now describe the full Phase 1-13 implemented surface, deployment checks, token behavior, file-backed data directories, and remaining production gaps.
- Local FLUX deployment correction completed after the documentation refresh: `backend/flux_service` now provides a local diffusers FLUX-compatible `/generate` service, `flux-local` is a first-class init provider, Docker Compose and Conda/tmux scripts run a local `flux` service by default, and docs now treat remote FLUX as compatibility rather than the main deployment path. This still does not bundle FLUX weights or implement high-resolution async regeneration.
