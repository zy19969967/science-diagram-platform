...
load_platform_env() {
  ...
  export GATEWAY_PORT="${GATEWAY_PORT:-19080}"
  export PLANNER_HOST="${PLANNER_HOST:-127.0.0.1}"
  export PLANNER_PORT="${PLANNER_PORT:-19081}"
  export POWERPAINT_HOST="${POWERPAINT_HOST:-127.0.0.1}"
  export POWERPAINT_PORT="${POWERPAINT_PORT:-19082}"
  export SEGMENTER_HOST="${SEGMENTER_HOST:-127.0.0.1}"
  export SEGMENTER_PORT="${SEGMENTER_PORT:-19083}"
  export FLUX_HOST="${FLUX_HOST:-127.0.0.1}"
  export FLUX_PORT="${FLUX_PORT:-19085}"
  export FLUX_INIT_URL="${FLUX_INIT_URL:-http://${FLUX_HOST}:${FLUX_PORT}}"
  export QWEN_IMAGE_HOST="${QWEN_IMAGE_HOST:-127.0.0.1}"
  export QWEN_IMAGE_PORT="${QWEN_IMAGE_PORT:-19086}"
  export QWEN_IMAGE_URL="${QWEN_IMAGE_URL:-http://${QWEN_IMAGE_HOST}:${QWEN_IMAGE_PORT}}"

  export FRONTEND_STATIC_HOST="${FRONTEND_STATIC_HOST:-0.0.0.0}"
  export FRONTEND_STATIC_PORT="${FRONTEND_STATIC_PORT:-19084}"
  ...
  export CONDA_ENV_FLUX="${CONDA_ENV_FLUX:-sci-flux}"
  export CONDA_ENV_QWEN_IMAGE="${CONDA_ENV_QWEN_IMAGE:-sci-qwen-image}"
  ...
  export PROJECT_GPU_POOL="${PROJECT_GPU_POOL:-0,1}"
  export QWEN_IMAGE_CUDA_VISIBLE_DEVICES="${QWEN_IMAGE_CUDA_VISIBLE_DEVICES:-0}"
  export POWERPAINT_CUDA_VISIBLE_DEVICES="${POWERPAINT_CUDA_VISIBLE_DEVICES:-1}"
  export PLANNER_CUDA_VISIBLE_DEVICES="${PLANNER_CUDA_VISIBLE_DEVICES:-1}"
  export SEGMENTER_CUDA_VISIBLE_DEVICES="${SEGMENTER_CUDA_VISIBLE_DEVICES:-1}"
  export FLUX_CUDA_VISIBLE_DEVICES="${FLUX_CUDA_VISIBLE_DEVICES:-1}"
  ...
  export FLUX_BACKEND="${FLUX_BACKEND:-diffusers}"
  export FLUX_MODEL_REPO="${FLUX_MODEL_REPO:-black-forest-labs/FLUX.2-klein-4B}"
  export FLUX_MODEL_DTYPE="${FLUX_MODEL_DTYPE:-bfloat16}"
  export FLUX_NUM_INFERENCE_STEPS="${FLUX_NUM_INFERENCE_STEPS:-4}"
  export FLUX_GUIDANCE_SCALE="${FLUX_GUIDANCE_SCALE:-1.0}"
  export FLUX_MAX_SEQUENCE_LENGTH="${FLUX_MAX_SEQUENCE_LENGTH:-512}"
  export FLUX_LOCAL_FILES_ONLY="${FLUX_LOCAL_FILES_ONLY:-false}"

  export QWEN_IMAGE_MODEL_REPO="${QWEN_IMAGE_MODEL_REPO:-Qwen/Qwen-Image-Edit}"
  export QWEN_IMAGE_MODEL_DTYPE="${QWEN_IMAGE_MODEL_DTYPE:-bfloat16}"
  export QWEN_IMAGE_NUM_INFERENCE_STEPS="${QWEN_IMAGE_NUM_INFERENCE_STEPS:-50}"
  export QWEN_IMAGE_TRUE_CFG_SCALE="${QWEN_IMAGE_TRUE_CFG_SCALE:-4.0}"
  export QWEN_IMAGE_STRENGTH="${QWEN_IMAGE_STRENGTH:-1.0}"
  export QWEN_IMAGE_LOCAL_FILES_ONLY="${QWEN_IMAGE_LOCAL_FILES_ONLY:-false}"
}
